...
import { InsertUser, users, items, InsertItem, pledges, InsertPledge } from "../drizzle/schema";
...
export async function insertItem(item: InsertItem) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(items).values(item);
}

export async function updateItemPledgeAmount(itemId: number, amount: number, isLocked: boolean) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getPledgesByItemId(itemId: number) {
  const db = await getDb();
  if (!db) return [];
...
export async function updatePledgeEmailStatus(pledgeId: number, sent: boolean) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.update(pledges)
